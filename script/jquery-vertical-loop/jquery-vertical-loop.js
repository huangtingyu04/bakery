...
(function (root, factory) {
    ...
    VerticalLoop.prototype = {
        ...
        _init: function () {
            ...
            this._to(0);
            this.container.parents(".marquee-box").on("mouseenter mousemove", function () {
                self.mouseIn = true;
                self.autoPause();
            });
            this.container.parents(".marquee-box").on("mouseleave", function () {
                self.mouseIn = false;
                if (self._isInScreen()) {
                    self.autoStart();
                }
            ...
            this.container.parent().find("i.clickup").on("click", function() {
                var goidx = ((self.index - 2) == -1) ? self.opt.total - 1 : self.index - 2;
                self._to(goidx);
                self._showCount(goidx);
            });

            this.container.parent().find("i.clickdown").on("click", function() {
                var goidx = (self.index == self.opt.total) ? 0 : self.index;
                self._to(goidx);
                self._showCount(goidx);
            });
        ...
        _showCount: function(idx) {
            idx = (idx === this.opt.total)? 0: idx;
            this.container.parent().find(".current").text(idx + 1);
        },
        _next: function () {
            this._to(this.index);
        },
        _to: function (index) {
            ...
            if (this.opt.showCount) this._showCount(index)
